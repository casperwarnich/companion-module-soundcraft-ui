...
class instance extends instance_skel {
    ...
    action(action) {
        ...
        switch (action.action) {
            ...
            case 'fade':
                ...
                break;
                 case 'auxfade':
                this.auxfade(opt.aux, opt.type, opt.channel, opt.level);           
    ...
    /**
     * Set the level for a defined channel on AUX (input/output/fx//etc.)
     * @param {char} type 
     * @param {int} channel 
     * @param {any} value 
     */
    auxfade(aux, type, channel, value) {
        var cmd = "3:::SETD^{type}.{channel}.aux.{aux}.value^{flag}"
            .replace("{aux}", aux - 1)
            .replace("{type}", type)
            .replace("{channel}", channel - 1)
            .replace("{flag}", value);
